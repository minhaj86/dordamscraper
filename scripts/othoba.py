...
logging.basicConfig(level=logging.DEBUG, filename="othoba.log")


def read_config():
    config = ConfigParser.ConfigParser()
    config.read("othoba.conf")
    global url_root
    global output_path
    global page_limit
...
def scrape_category(category):
    ...
    logging.debug("Requested Page status code : '{0}'".format(body.status_code))
    if body.status_code == 503:
        logging.debug("Requested Page unavailable right now, sleeping for 6 seconds")
        time.sleep(6)
    logging.info("Requesting page again: {0}".format(url))
    body = requests.get(url)
    logging.debug("Requested Page status code : '{0}'".format(body.status_code))
    logging.debug("Requested Page: \n{0}".format(body.content))
...
def scrape_all():
    categories = get_categories("othoba.cat")
